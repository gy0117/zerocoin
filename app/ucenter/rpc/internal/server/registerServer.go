// Code generated by goctl. DO NOT EDIT.
// Source: register.proto

package server

import (
	"context"

	"ucenter-rpc/internal/logic"
	"ucenter-rpc/internal/svc"
	"grpc-common/ucenter/types/register"
)

type RegisterServer struct {
	svcCtx *svc.ServiceContext
	register.UnimplementedRegisterServer
}

func NewRegisterServer(svcCtx *svc.ServiceContext) *RegisterServer {
	return &RegisterServer{
		svcCtx: svcCtx,
	}
}

func (s *RegisterServer) RegisterByPhone(ctx context.Context, in *register.RegisterReq) (*register.RegisterResp, error) {
	l := logic.NewRegisterByPhoneLogic(ctx, s.svcCtx)
	return l.RegisterByPhone(in)
}


func (s *RegisterServer) SendCode(ctx context.Context, in *register.CodeReq) (*register.CodeResp, error) {
	l := logic.NewRegisterByPhoneLogic(ctx, s.svcCtx)
	return l.SendCode(in)
}
