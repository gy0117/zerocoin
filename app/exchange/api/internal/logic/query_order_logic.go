...
func (l *QueryOrderLogic) QueryHistoryOrders(req *types.ExchangeReq) (*page.PageData, error) {
	...
	orderResp, err := l.svcCtx.OrderRpc.QueryHistoryOrders(ctx, &order.OrderReq{
		Ip:       req.Ip,
		Symbol:   req.Symbol,
		Page:     req.PageNo,
...
func (l *QueryOrderLogic) QueryCurrentOrders(req *types.ExchangeReq) (*page.PageData, error) {
	ctx, cancel := context.WithTimeout(l.ctx, time.Second*10)
	defer cancel()
	...
	orderResp, err := l.svcCtx.OrderRpc.QueryCurrentOrders(ctx, &order.OrderReq{
		Ip:       req.Ip,
		Symbol:   req.Symbol,
		Page:     req.PageNo,
		PageSize: req.PageSize,
...
func (l *QueryOrderLogic) QueryCompleteOrders(req *types.ExchangeReq) (*page.PageData, error) {
	ctx, cancel := context.WithTimeout(l.ctx, time.Second*10)
	defer cancel()
