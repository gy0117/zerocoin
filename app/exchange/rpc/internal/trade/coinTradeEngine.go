...
const (
	topicExchangeOrderTradePlate = "exchange_order_trade_plate"
...
type CoinTradeEngine struct {
	...
	marketTradeOperate *MarketTradeOperate
	limitTradeOperate  *LimitTradeOperate
...
func (engine *CoinTradeEngine) Trade(order *model.ExchangeOrder) {
	logx.Info("start buy&sell trade plate")
...
func (engine *CoinTradeEngine) sendTradePlateData(plate *plate.TradePlate) {
	...
	logx.Info("CoinTradeEngine | sendTradePlateData success")
}
	...
	for _, v := range list.List {
		for _, matchOrder := range v.List {
			...
			if curOrder.Direction == model.DirectionBuyInt {
				...
				if curOrder.Price < matchOrder.Price {
					break
				}
			}
			if curOrder.Direction == model.DirectionSellInt {
	...
	for _, v := range list.List {
		for _, matchOrder := range v.List {
			...
			if curOrder.Direction == model.DirectionBuyInt {
				...
				unTradedAmount = operate.DivFloor(tmp, price, 8)
			}
			if matchAmount >= unTradedAmount {
				// 能满足订单，直接交易
				matchOrder.TradedAmount = operate.AddFloor(matchOrder.TradedAmount, unTradedAmount, 8)
