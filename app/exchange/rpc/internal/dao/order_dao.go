...
func (dao *OrderDao) UpdateOrderComplete(ctx context.Context, orderId string, tradedAmount float64, turnover float64) error {
	...
	return session.Model(&model.ExchangeOrder{}).Exec(sql, tradedAmount, turnover, model.StatusCompleted, orderId).Error
}
...
func (dao *OrderDao) FindTradingOrderBySymbol(ctx context.Context, symbol string) (list []*model.ExchangeOrder, err error) {
	session := dao.conn.Session(ctx)
	err = session.Model(&model.ExchangeOrder{}).Where("symbol=? and status=?", symbol, model.StatusTrading).Find(&list).Error
...
func (dao *OrderDao) CancelOrder(ctx context.Context, orderId string) error {
	session := dao.conn.Session(ctx)
	err := session.Model(&model.ExchangeOrder{}).Where("order_id=?", orderId).Update("status", model.StatusCanceled).Error
	if err == gorm.ErrRecordNotFound {
		return nil
	}
	return err
...
func (dao *OrderDao) FindCurrentTradingCount(ctx context.Context, memId int64, symbol string, direction int) (total int64, err error) {
	session := dao.conn.Session(ctx)
	err = session.Model(&model.ExchangeOrder{}).Where("user_id=? and symbol=? and direction=? and status=?", memId, symbol, direction, model.StatusTrading).Count(&total).Error
	if err == gorm.ErrRecordNotFound {
		return 0, nil
	}
	return
...
func (dao *OrderDao) GetCurrentOrder(ctx context.Context, symbol string, memId int64, pageNo int64, pageSize int64) (list []*model.ExchangeOrder, total int64, err error) {
	session := dao.conn.Session(ctx)
	// 找到第pageNo页的数据，一页的数据量为pageSize
	err = session.Model(&model.ExchangeOrder{}).Where("symbol=? and user_id=? and status=?", symbol, memId, model.StatusTrading).Limit(int(pageSize)).Offset(int((pageNo - 1) * pageSize)).Find(&list).Error
	if err == gorm.ErrRecordNotFound {
		return nil, 0, nil
	}
	err = session.Model(&model.ExchangeOrder{}).Where("symbol=? and user_id=?  and status=?", symbol, memId, model.StatusTrading).Count(&total).Error
	return
}
